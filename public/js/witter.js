    ...
    async function renderWitRow(witData, user) {
        ...
        $.ajax({
            ...
            success: function (response) {
                const followBtnText = response.followers && response.followers.includes(loggedInUser) ? 'Following' : 'Not Following';
                $(`#wit-${witData.id} .following-status`).text(followBtnText);
                
                const popupFollowBtnText = response.followers && response.followers.includes(loggedInUser) ? 'Unfollow' : 'Follow';
                $(`.follow-btn-${witData.author}`).text(popupFollowBtnText);

                // Update the .following-status button with the correct status
                $(`#wit-${witData.id} .following-status`).text(followBtnText);

                // console.log(`#wit-${witData.id} .following-status`, $(`#wit-${witData.id} .following-status`));
                // $(`#wit-${witData.id} .following-status`).text(followBtnText);
            ...
            if (data.length !== 0) {
                ...
                Promise.all(profilePicturePromises).then(profilePictures => {
                    data.forEach((witData, i) => {
                        ...
                        // ==================================
                        // AJAX CALL FOR THE FOLLOWING CHECK
                        // ==================================
                        $.ajax({
                            method: 'GET',
                            url: `/api/users/${witData.author}/followers`,  // API endpoint
